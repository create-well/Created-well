...
const usernameSeed = await read('scripts/seed-username-map.mjs');
...
assert(![server, dashboard, notionWriter, usernameSeed].some((source) => source.includes('kv_store_8dcd9693')), 'deprecated KV table identifier found in API runtime files or seed scripts');
assert(usernameSeed.includes("KV_TABLE          = 'kv_store_dabe1c74'"), 'username seed must use the canonical KV table');
...
for (const envKey of ['NOTION_SECRET', 'NOTION_DB_MOVES', 'NOTION_DB_PEOPLE', 'NOTION_DB_FLOWS', 'NOTION_DB_CONTENT', 'NOTION_DB_MONEY', 'SUPABASE_URL', 'SUPABASE_SERVICE_ROLE_KEY']) {
...
assert(!envExample.match(/NOTION_SECRET=(?!secret_your_notion_token_here$).+/m), '.env.local.example must not contain a real Notion secret');
